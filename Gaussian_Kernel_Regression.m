...
for testIndex = 1:numTests
    ...
    for trialIndex = 1:numTrials
        ...
        estimatedChannel = performGaussianKernelRegression(pilotIndices, rawEstimates, totalSubcarriers, currentSigma);
...
for symbolIndex = 1:numOFDMSymbols
    ...
    % Perform kernel regression to estimate the channel
    estimatedChannel = performGaussianKernelRegression(pilotIndices, rawChannelEstimates, totalSubcarriers, kernelWidth);
...
ylabel('OFDM Symbol Index');
zlabel('|h[n,k]|');
title('Gaussian Kernel Regression Channel Estimates');
colorbar;
...
for symbolIndex = 1:symbolsToPlot
    ...
    legend('True Channel', 'Gaussian Kernel Regression Estimate', 'Location', 'Best');
